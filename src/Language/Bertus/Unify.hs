...
unify :: Monad m => ProbId -> Eqn v -> BMonadT v m ()
...
simplify pid prob probs = go probs []
  where
    ...
    subgoal :: Monad m => Problem v -> (ProbId -> BMonadT v m a) -> BMonadT v m a
    subgoal prob' f =
        do pid' <- probId <$> fresh
...
pendingSolve :: ProbId -> Problem v -> [ProbId] -> BMonadT v m ()
pendingSolve = undefined
