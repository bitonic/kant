...
unrollArr :: TermSyn -> ([(Maybe Id, TermSyn)], TermSyn)
unrollArr (Arr ty s) = ((n, ty) : pars, t₂)
  where (n, t₁) = scopeN s; (pars, t₂) = unrollArr t₁
unrollArr t = ([], t)

unrollLam :: TermSyn -> ([Maybe Id], TermSyn)
unrollLam (Lam s) = (vn : vs, t₂)
  where (vn, t₁) = scopeN s; (vs, t₂) = unrollLam t₁
unrollLam t = ([], t)

unrollAnn :: TermSyn -> TermSyn -> ([(Maybe Id, TermSyn)], TermSyn, TermSyn)
